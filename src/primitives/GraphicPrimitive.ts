...
export abstract class GraphicPrimitive {
    ...
    // Scratch MapCoordinates for hot-path distance calculations (avoids allocation).
    protected scratchCoord: MapCoordinates = new MapCoordinates();
    ...
    getCurrentLayer(): LayerDesc | null {
    ...
    protected drawText(
        ...
        if (this.changed) {
            ...
            this._tc_tth = Math.round(this._tc_th / coordSys.getYMagnitude());
        ...
        if (
            !g.hitClip(this._tc_xa, this._tc_ya, this._tc_w1, this._tc_th) &&
        ...
        return (
            (this.name !== '' &&
                GeometricDistances.pointInRectangle(
                    this.virtualPoint[nvp]!.x,
                    this.virtualPoint[nvp]!.y,
                    this._tc_tw1,
                    this._tc_tth,
                    px,
                    py,
            ...
            (this.value !== '' &&
                GeometricDistances.pointInRectangle(
                    this.virtualPoint[vvp]!.x,
                    this.virtualPoint[vvp]!.y,
                    this._tc_tw2,
                    this._tc_tth,
                    px,
                    py,
    ...
    setValue(tokens: string[], nn: number): void {
        this.changed = true;
        if (tokens[0] !== 'TY')
            throw new Error(`Invalid primitive: ${tokens[0]} programming error?`);
    ...
    setDrawOnlyLayer(_i: number): void {
        /* overridden by PrimitiveMacro */
    }
    ...
    protected selectLayer(
        ...
        if (this.selectedState) {
            ...
            if (layerColor) {
                ...
                const colorChanged =
                    currentColor.getRed() !== layerColor.getRed() ||
                    currentColor.getGreen() !== layerColor.getGreen() ||
                    currentColor.getBlue() !== layerColor.getBlue();
                // Use per-render ctx if provided, else fall back to static
                const oldAlpha = renderCtx ? renderCtx.alpha : GraphicPrimitive.oldalpha;
                const alphaChanged = oldAlpha !== layerAlpha;
                if (colorChanged || alphaChanged) {
                    g.setColor(layerColor);
                    g.setAlpha(layerAlpha);
                    if (renderCtx) {
                        renderCtx.alpha = layerAlpha;
                    } else {
    ...
    setDrawOnlyPads(_t: boolean): void {
        /* overridden by PrimitiveMacro and PrimitivePCBPad */
    }
