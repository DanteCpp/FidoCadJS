/**
 * @file PrimitiveRectangle.ts
 * @author Dante Loi
 * @date 2026-04-24
 * @brief Rectangle primitive (empty or filled)
...
export class PrimitiveRectangle extends GraphicPrimitive {
    ...
    getControlPointNumber(): number {
        return PrimitiveRectangle.N_POINTS;
    }
    ...
    getDistanceToPoint(px: number, py: number): number {
        ...
        if (this.isFilled) {
            return GeometricDistances.pointInRectangle(xa, ya, xb - xa, yb - ya, px, py)
                ? PrimitiveRectangle.DISTANCE_IN
                : PrimitiveRectangle.DISTANCE_OUT;
        }
    ...
    override intersects(rect: RectangleG, isLeftToRightSelection: boolean): boolean {
        ...
        return (
            ...
            rect.intersectsLine(x1, y2, x2, y2) ||
            rect.intersectsLine(x1, y1, x1, y2) ||
            rect.intersectsLine(x2, y1, x2, y2)
        );
    }
}
