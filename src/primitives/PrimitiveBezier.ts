/**
 * @file PrimitiveBezier.ts
 * @author Dante Loi
 * @date 2026-04-24
 * @brief Cubic Bézier curve primitive
...
export class PrimitiveBezier extends GraphicPrimitive {
    ...
    getControlPointNumber(): number {
        return PrimitiveBezier.N_POINTS;
    }
    ...
    private drawArrow(
        ...
        const psx = this.virtualPoint[aa]!.x,
            psy = this.virtualPoint[aa]!.y;
        let pex: number, pey: number;
        if (
            this.virtualPoint[aa]!.x !== this.virtualPoint[bb]!.x ||
            this.virtualPoint[aa]!.y !== this.virtualPoint[bb]!.y
        ) {
            pex = this.virtualPoint[bb]!.x;
            pey = this.virtualPoint[bb]!.y;
        } else if (
            this.virtualPoint[aa]!.x !== this.virtualPoint[cc]!.x ||
            this.virtualPoint[aa]!.y !== this.virtualPoint[cc]!.y
        ) {
            pex = this.virtualPoint[cc]!.x;
            pey = this.virtualPoint[cc]!.y;
        } else {
            pex = this.virtualPoint[dd]!.x;
            pey = this.virtualPoint[dd]!.y;
        }
        return this.arrowData.drawArrow(
            g,
    ...
    draw(g: GraphicsInterface, coordSys: MapCoordinates, layerV: LayerDesc[]): void {
        ...
        if (this.changed) {
            ...
            this.xmin = r.x - h;
            this.ymin = r.y - h;
            this.width = r.width + 2 * h;
            this.height = r.height + 2 * h;
        }
    ...
    parseTokens(tokens: string[], nn: number): void {
        ...
        this.virtualPoint[2]!.x = Globals.coord(tokens[5]);
        this.virtualPoint[2]!.y = Globals.coord(tokens[6]);
        this.virtualPoint[3]!.x = Globals.coord(tokens[7]);
        this.virtualPoint[3]!.y = Globals.coord(tokens[8]);
        this.virtualPoint[this.getNameVirtualPointNumber()]!.x = x1 + 5;
        this.virtualPoint[this.getNameVirtualPointNumber()]!.y = y1 + 5;
        this.virtualPoint[this.getValueVirtualPointNumber()]!.x = x1 + 5;
        this.virtualPoint[this.getValueVirtualPointNumber()]!.y = y1 + 10;
        ...
        if (this.arrowData.atLeastOneArrow()) {
            ...
            if (this.arrowData.isArrowStart()) {
                t = this.arrowData.isInArrow(
                    ...
                    this.virtualPoint[1]!.x,
                    this.virtualPoint[1]!.y,
            ...
            if (this.arrowData.isArrowEnd()) {
                r = this.arrowData.isInArrow(
                    px,
                    py,
                    this.virtualPoint[3]!.x,
                    this.virtualPoint[3]!.y,
                    this.virtualPoint[2]!.x,
                    this.virtualPoint[2]!.y,
    ...
    toString(extensions: boolean): string {
        ...
        let s =
            `BE ${c(this.virtualPoint[0]!.x)} ${c(this.virtualPoint[0]!.y)} ` +
            `${c(this.virtualPoint[1]!.x)} ${c(this.virtualPoint[1]!.y)} ` +
            `${c(this.virtualPoint[2]!.x)} ${c(this.virtualPoint[2]!.y)} ` +
            `${c(this.virtualPoint[3]!.x)} ${c(this.virtualPoint[3]!.y)} ${this.getLayer()}\n`;
        if (
            extensions &&
    ...
    override intersects(rect: RectangleG, isLeftToRightSelection: boolean): boolean {
        ...
        const segments = 100;
        let px = this.virtualPoint[0]!.x,
            py = this.virtualPoint[0]!.y;
        for (let i = 1; i <= segments; i++) {
            const t = i / segments;
            const [cx, cy] = this._bezierPoint(
                t,
                this.virtualPoint[0]!.x,
                this.virtualPoint[0]!.y,
                this.virtualPoint[1]!.x,
