/**
 * @file PrimitiveOval.ts
 * @author Dante Loi
 * @date 2026-04-24
 * @brief Oval / ellipse primitive
...
export class PrimitiveOval extends GraphicPrimitive {
    ...
    draw(g: GraphicsInterface, coordSys: MapCoordinates, layerV: LayerDesc[]): void {
        ...
        if (this.changed) {
            ...
            this.xa = Math.min(this.x1, this.x2);
            this.xb = Math.max(this.x1, this.x2);
            this.ya = Math.min(this.y1, this.y2);
            this.yb = Math.max(this.y1, this.y2);
        ...
        if (!g.hitClip(this.xa, this.ya, this.xb - this.xa + 1, this.yb - this.ya + 1)) return;
        ...
        if (this.isFilled) {
            ...
            if (this.xa !== this.xb && this.ya !== this.yb) {
                ...
                g.drawLine(this.xa, this.ya, this.xb, this.yb);
            }
        }
    }
    ...
    parseTokens(tokens: string[], nn: number): void {
        this.changed = true;
        if (tokens[0] !== 'EV' && tokens[0] !== 'EP')
            throw new Error(`EV/EP: Invalid primitive: ${tokens[0]}`);
        if (nn < 5) throw new Error('Bad arguments on EV/EP');
    ...
    getDistanceToPoint(px: number, py: number): number {
        ...
        if (this.isFilled) {
            return GeometricDistances.pointInEllipse(xa, ya, xb - xa, yb - ya, px, py) ? 0 : 1000;
        }
        return GeometricDistances.pointToEllipse(xa, ya, xb - xa, yb - ya, px, py);
    }

    toString(extensions: boolean): string {
        const cmd = this.isFilled ? 'EP' : 'EV';
        const c = Globals.formatCoord;
        let s =
            `${cmd} ${c(this.virtualPoint[0]!.x)} ${c(this.virtualPoint[0]!.y)} ` +
            `${c(this.virtualPoint[1]!.x)} ${c(this.virtualPoint[1]!.y)} ${this.getLayer()}\n`;
    ...
    override intersects(rect: RectangleG, isLeftToRightSelection: boolean): boolean {
        ...
        const ovalBounds = new RectangleG(x1, y1, x2 - x1, y2 - y1);
        if (!rect.intersects(ovalBounds)) return false;
        const cx = (x1 + x2) / 2,
            cy = (y1 + y2) / 2;
        const a = (x2 - x1) / 2,
            b = (y2 - y1) / 2;
        ...
        const step = 0.1;
        for (let i = rect.getX(); i <= rect.getX() + rect.getWidth(); i += step) {
            for (let j = rect.getY(); j <= rect.getY() + rect.getHeight(); j += step) {
                const nx = (i - cx) / a,
                    ny = (j - cy) / b;
                if (Math.abs(nx * nx + ny * ny - 1) < 0.001) return true;
            }
        }
        return false;
    }
