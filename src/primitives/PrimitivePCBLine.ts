/**
 * @file PrimitivePCBLine.ts
 * @author Dante Loi
 * @date 2026-04-24
 * @brief PCB trace line primitive
...
export class PrimitivePCBLine extends GraphicPrimitive {
    ...
    private xbpap1: number = 0;
    private ybpap1: number = 0;
    ...
    getControlPointNumber(): number {
        return PrimitivePCBLine.N_POINTS;
    }
        ...
        if (this.changed) {
            ...
            this.wiPix = Math.abs(
                coordSys.mapXr(this.virtualPoint[0]!.x, this.virtualPoint[0]!.y) -
                    coordSys.mapXr(
                        this.virtualPoint[0]!.x + this.width,
    ...
    toString(extensions: boolean): string {
        ...
        let s =
            `PL ${c(this.virtualPoint[0]!.x)} ${c(this.virtualPoint[0]!.y)} ` +
    ...
    override intersects(rect: RectangleG, isLeftToRightSelection: boolean): boolean {
        ...
        const x1 = this.virtualPoint[0]!.x,
            y1 = this.virtualPoint[0]!.y;
        const x2 = this.virtualPoint[1]!.x,
            y2 = this.virtualPoint[1]!.y;
