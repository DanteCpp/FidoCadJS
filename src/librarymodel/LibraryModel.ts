...
export class IllegalLibraryAccessException extends LibraryException {
    constructor(message: string) {
        super(message);
        this.name = 'IllegalLibraryAccessException';
    }
}
...
export class IllegalNameException extends LibraryException {
    constructor(message: string) {
        super(message);
        this.name = 'IllegalNameException';
    }
}
...
export class LibraryModel {
    ...
    getAllMacros(): Map<string, MacroDesc> {
    ...
    remove(target: Library | Category | MacroDesc): void {
        if (target instanceof Library) {
            ...
            if (parentLib.isStdLib())
                throw new IllegalLibraryAccessException(
                    "A category in a standard library can't be removed.",
                );
            ...
            if (category.getParentLibrary().isStdLib())
                throw new IllegalLibraryAccessException(
                    "A macro in a standard library can't be removed.",
                );
    ...
    rename(target: Library | Category | MacroDesc, newName: string): void {
        ...
        if (target instanceof Library) {
            ...
            if (target.getParentLibrary().isStdLib())
                throw new IllegalLibraryAccessException(
                    "A category in a standard library can't be renamed.",
                );
            target.setName(newName);
            ...
            if (this.isStdLib(target))
                throw new IllegalLibraryAccessException(
                    "A macro in a standard library can't be renamed.",
                );
            target.name = newName;
            this.save();
        }
    ...
    changeKey(macro: MacroDesc, newKey: string): void {
        if (!newKey || newKey.trim().length === 0)
            throw new IllegalKeyException('Key must not be empty.');
        if (LibUtils.checkKeyInvalidChars(newKey))
            throw new IllegalKeyException('Key must not contain spaces, dots, or ].');
        if (this.isStdLib(macro))
            throw new IllegalLibraryAccessException(
                "A macro in a standard library can't have its key changed.",
            );
    ...
    private copyMacro(macro: MacroDesc, destCategory: Category): MacroDesc {
        ...
        newMacro.key = LibraryModel.createMacroKey(
            destCategory.getParentLibrary().getFilename(),
            newPlainKey,
        );
