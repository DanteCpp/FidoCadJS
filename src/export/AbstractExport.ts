...
export abstract class AbstractExport implements ExportInterface {
    ...
    protected dashArray(style: number): number[] {
        ...
        switch (style) {
            case 1: return [u * 2, u * 2];
            case 2: return [u * 2, u * 2, u, u * 2];
            case 3: return [u * 3, u * 2];
            case 4: return [u, u];
            default: return [];
        }
    }
    ...
    abstract exportAdvText(x: number, y: number, sizex: number, sizey: number,
        fontname: string, isBold: boolean, isMirrored: boolean, isItalic: boolean,
        orientation: number, layer: number, text: string): void;

    abstract exportBezier(x1: number, y1: number, x2: number, y2: number,
    ...
    abstract exportLine(x1: number, y1: number, x2: number, y2: number, layer: number,
        arrowStart: boolean, arrowEnd: boolean, arrowStyle: number,
        arrowLength: number, arrowHalfWidth: number,
        dashStyle: number, strokeWidth: number): void;

    abstract exportMacro(x: number, y: number, isMirrored: boolean,
    ...
    abstract exportOval(x1: number, y1: number, x2: number, y2: number,
        isFilled: boolean, layer: number,
        dashStyle: number, strokeWidth: number): void;

    abstract exportPCBLine(x1: number, y1: number, x2: number, y2: number,
        width: number, layer: number): void;

    abstract exportPCBPad(x: number, y: number, style: number,
        six: number, siy: number, indiam: number,
        layer: number, onlyHole: boolean): void;

    abstract exportPolygon(vertices: PointDouble[], nVertices: number,
        isFilled: boolean, layer: number,
        dashStyle: number, strokeWidth: number): void;

    abstract exportCurve(vertices: PointDouble[], nVertices: number,
        isFilled: boolean, isClosed: boolean, layer: number,
        arrowStart: boolean, arrowEnd: boolean, arrowStyle: number,
        arrowLength: number, arrowHalfWidth: number,
        dashStyle: number, strokeWidth: number): boolean;

    abstract exportRectangle(x1: number, y1: number, x2: number, y2: number,
        isFilled: boolean, layer: number,
        dashStyle: number, strokeWidth: number): void;

    abstract exportArrow(x: number, y: number, xc: number, yc: number,
