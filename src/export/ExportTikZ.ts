 ...
 * @author Dante Loi
 * @date 2026-05-09
 * @brief TikZ export for modern LaTeX documents (built on PGF, more idiomatic)
...
export class ExportTikZ implements ExportInterface {
    ...
    exportPolygon(
        ...
        const points = vertices.slice(0, nVertices).map(v =>
            `(${this.cLe(v.x)},${this.cLe(v.y)})`
        );
        this.buffer.push(
            `${cmd}[${opts}] ${points.join(' -- ')} -- cycle;`
        );
    }
    ...
    exportPCBPad(
        ...
        if (onlyHole) {
            ...
            switch (style) {
                ...
                case 2: { // Rounded pad — rendered as square in TikZ
                    const xdd = x - six / 2.0;
                    const ydd = y - siy / 2.0;
                    this.buffer.push(
                        `\\filldraw[${opts}] (${this.cLe(xdd)},${this.cLe(ydd)}) ` +
