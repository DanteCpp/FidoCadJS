...
import {
  gotoApp, loadCircuit, primitiveCount, pressKey,
} from './utils';
...
test.describe('Menu Bar — File Menu', () => {
  ...
  test('File menu opens and shows items', async ({ page }) => {
    ...
    await expect(page.locator('div', { hasText: 'Open' }).first()).toBeVisible();
    await expect(page.locator('div', { hasText: 'Save FCD' }).first()).toBeVisible();
  });
  ...
  test('View menu shows Zoom and Options', async ({ page }) => {
    ...
    await expect(page.locator('div', { hasText: 'Zoom Fit' }).first()).toBeVisible();
  });
